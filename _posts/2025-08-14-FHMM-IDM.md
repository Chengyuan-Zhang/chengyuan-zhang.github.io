...
The **joint state** is $$z_t=(z_t^{[B]},z_t^{[S]})$$ in the product space
$$\mathcal{Z}=\{1,\dots,K^{[B]}\}\times\{1,\dots,K^{[S]}\}$$.
Let $$k=(k^{[B]},k^{[S]})$$ index a joint state. The transition matrix is
$$\Pi\in\mathbb{R}^{|\mathcal{Z}|\times|\mathcal{Z}|}$$ with rows that sum to 1:
$$
